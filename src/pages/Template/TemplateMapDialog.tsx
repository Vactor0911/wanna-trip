...
import NaverMap from "../../components/naver_map/NaverMap";
import { TemplateInterface } from "../../state/template";
...
const TemplateMapDialog = (props: TemplateMapDialogProps) => {
  ...
  const displayMarkers = useMemo(() => {
    const locationsToShow = showAllLocations
      ...
      : selectedLocation
      ? allLocations.filter(
          (loc) => loc.dayNumber === selectedLocation.dayNumber
        )
    ...
    return locationsToShow
      ...
      .map((location, index) => ({
        ...
        label: showAllLocations
          ? location.dayNumber.toString()
          : (index + 1).toString(),
      }));
  }, [
    selectedLocation,
  ...
  const displayPolylines = useMemo(() => {
    const createPolyline = (
      ...
      return {
        path: sortedLocations.map((location) => ({
          lat: location.lat,
          lng: location.lng,
        })),
    ...
    if (showAllLocations) {
      ...
      return Object.entries(groupedByDay)
        .map(([dayNumber, locations]) =>
          createPolyline(locations, parseInt(dayNumber))
        )
        .filter(
          (polyline): polyline is NonNullable<typeof polyline> =>
            polyline !== null
        );
    } else {
  ...
  const handleLocationClick = useCallback(
    (location: (typeof allLocations)[0], event?: React.MouseEvent) => {
  ...
  const renderScheduleList = useCallback(() => {
    if (
      ...
      (!showAllLocations && selectedDay && !selectedDayLocations.length)
    ) {
      return (
        <Box
          sx={{
        ...
        {scheduleByDay[1] && (
          <Paper
            ...
            sx={{
              p: 2,
              border:
                selectedDay === 1 && !showAllLocations
                  ? `2px solid ${getDayColor(1)}`
                  : "none",
              cursor: "pointer",
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* 스크롤 가능한 콘텐츠 */}
          <Box sx={{ p: 2, pt: 1 }}>{renderScheduleList()}</Box>
        </Box>
              ...
              >
                ...
                >
                  <Stack direction="row" alignItems="center" spacing={1}>
                    ...
                    <Tooltip
                      title={showAllLocations ? "전체보기 중" : "전체보기"}
                    >
                      <span>
                        <IconButton
                          onClick={handleToggleAllLocations}
